...
- **Agent 协作协议**：验收清单、逐条证据、带类型的阻塞、交接步骤列表；协议规则通过 MCP 直接发给 Agent（[AGENT_PROTOCOL](docs/AGENT_PROTOCOL.md)）。
- **人与 Agent 的边界**：授权、验收、叫停由人决定；可以暂停整个项目或单个连接的 Agent；Agent 每天打扰每个人的次数有上限；“我的 Agent”页面和按人与 Agent 分开统计的协作指标。
- **AI 辅助（可选）**：提交预审、任务简报、动态摘要、验收清单起草、推荐人选；接入任意 OpenAI 兼容接口，只给建议，不改变任务状态。
...
- 服务端 AI 辅助默认关闭。开启后，开启了 AI 的项目会把全体成员可见的文字发送到管理员配置的模型服务（受限成果和草稿除外），每次调用都记账。
