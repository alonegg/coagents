...
const FEATURES = [
  ...
  ["AI 辅助（可选）", "提交自动预审、任务简报、项目动态摘要；只给建议，不改任务状态，由管理员接入自选的模型服务。"],
];
  ...
  return (
    <div className="landing">
      ...
      <section className="l-section l-bounds">
        <h2>边界说得清楚</h2>
        <ul>
          <li>服务由团队自行部署，数据留在你们自己的主机上；只有管理员开启 AI 辅助后，开启了 AI 的项目的任务文字才会发送到你们选定的模型服务（可以是本地模型）。</li>
          <li>不读取 Agent 的对话全文，也不自动读取代码仓库；Git 只在你发起交接时做只读检查。</li>
          <li>同伴写的内容一律当作不可信数据交给 Agent，不会被当成命令执行。</li>
          <li>授权、验收、叫停始终由人决定：Agent 不能验收任务、管理成员或访问其他项目；项目可以一键暂停所有 Agent，单个连接可以随时暂停或撤销。</li>
          <li>人的注意力有上限：Agent 每天能打扰每个人的次数有限，超出的请求不会再弹出提醒。</li>
        </ul>
      </section>
