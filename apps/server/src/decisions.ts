...
export function listDecisions(ctx: AppContext, projectId: string, includeHistory: boolean): DecisionView[] {
  const rows = ctx.db
    .prepare(
      `SELECT d.id, d.body, d.supersedes_id, n.id AS superseded_by, d.created_by, u.display_name AS created_by_name, d.created_at, d.event_seq
       FROM decisions d JOIN users u ON u.id = d.created_by LEFT JOIN decisions n ON n.supersedes_id = d.id
...
export function publishGeneralBlocker(ctx: AppContext, projectId: string, actor: Actor, body: string): number {
