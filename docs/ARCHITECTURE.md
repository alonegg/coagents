...
全文提取覆盖 Markdown、纯文本和有文本层 PDF，异步任务显示索引状态。中文检索是已知风险：SQLite FTS5 默认分词不切中文，trigram 分词对两字词不命中；M7 开始前需用中文语料对比 trigram 加短词回退与应用侧分词两种方案，按召回和权限过滤成本选定。查询阶段连接当前成果权限，随后生成片段和总数；不能先返回全文片段再在 UI 隐藏。删除/撤权立即过滤，即使旧索引稍后清理。历史索引需显式选版本，索引恢复可从受管成果重建。
