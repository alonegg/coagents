...
| 协议 v2（2026-09-24） | `claude -p` 只收到“按 CoAgents 协议完成任务 X”一句话，按 server instructions 走完 get_context → claim → get_task → submit（逐条证据）；被退回后读出原因，修复并推送后，用列表形式的下一步交接给指定成员。见 [验收记录](acceptance.md) |
